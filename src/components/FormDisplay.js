...
const DynamicForm = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...formFields.reduce((acc, field, index) => {
      ...
      if (field.type === 'email') {
        acc[fieldName] = acc[fieldName].email('Invalid email address');
      }
      return acc;
    }, {}),
  });
    ...
    return formFields && formFields.map((field, index) => {
      ...
      switch (type) {
        case 'text':
            ...
            <span style={{ color: 'red', marginLeft:'16px' }}>{formik.errors[fieldName]}</span>
          )}
                </div>
  ...
  return (
    <div className='form_display'>
      ...
      <form onSubmit={formik.handleSubmit}>
        {renderFormFields()}
        <button className='submit_btn display' type="submit" onClick={()=> formik.handleSubmit(formik.values)}>Submit</button>
      </form>
    </div>
  );
};
